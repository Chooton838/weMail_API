 ...
 *
 */
test("Test-10: Log into WP-Site", async ({ request }) => {
  const basePage = new BasePage(request);

  await basePage.wordpress_site_login();
  
});

test("Test-10.1: Log into WP-Site", async ({ request }) => {
  const basePage = new BasePage(request);

  await basePage.visitContactForm7();
  
});

// test("Test-11: Setup - 'Contact Form 7'...", async ({ request }) => {
//   //Create New List
//   const list = new Rat_ListsPage(request);
//   const integrations = new Rat_IntegrationsPage(request);

//   const contactForm_ListName = `Contact-Form-7 [001]`;
//   const list_created_id = await list.list_create(contactForm_ListName);
//   data.form_data.list_id = list_created_id;

//   //Setup - 'Contact Form 7'...
//   await integrations.setup_Contact_Forms_7(data.form_data.list_id);
// });
