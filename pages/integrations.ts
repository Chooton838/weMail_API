...
export class IntegrationsPage {
	...
	async create_affiliate(affiliate_username: string) {
		...
		await this.page.locator(selector.integration_create_affiliate.username).fill(affiliate_username);
		await this.page.waitForTimeout(2000);

		await this.page.locator(selector.integration_create_affiliate.select_affiliate(affiliate_username)).click();

		await this.page.locator(selector.integration_create_affiliate.affiliate_status).selectOption("Pending");
		await this.page.locator(selector.integration_create_affiliate.affiliate_rate_type).click();
		await this.page.locator(selector.integration_create_affiliate.affiliate_rate).fill("5");
		await this.page.locator(selector.integration_create_affiliate.affiliate_submit).click();
	...
	async approve_affiliate(affiliate_username: string) {
		...
		await this.page.goto(selector.integration_affiliate_actions.affiliates_list_page_url, { waitUntil: "networkidle" });

		await this.page.locator(selector.integration_affiliate_actions.select_affiliate(affiliate_username)).click();

		await this.page.locator(selector.integration_affiliate_actions.select_bulk_action).selectOption("Accept");
		await this.page.locator(selector.integration_affiliate_actions.do_action).click();
	...
	async delete_affiliate(affiliate_username: string) {
		...
		await this.page.locator(selector.integration_affiliate_actions.select_bulk_action).selectOption("Delete");
		await this.page.locator(selector.integration_affiliate_actions.do_action).click();
		await this.page.waitForLoadState("networkidle");
	...
	async create_contact_forms_7(contact_form_7_name: string) {
		...
		await this.page.locator(selector.integration_create_cf7.title).fill(contact_form_7_name);
		await this.page.locator(selector.integration_create_cf7.save).click();
		await this.page.waitForTimeout(3000);
		expect(await this.page.locator(selector.integration_create_cf7.shortcode).isVisible()).toBeTruthy();
	}
	...
	async map_contact_form_7(list_name: string, contact_form_7_name: string) {
		...
		await this.page.locator(selector.integration_map_cf7.check_cf7(contact_form_7_name)).click();

		await this.page.locator(selector.integration_map_cf7.select_cf7(contact_form_7_name)).click();

		await this.page.locator(selector.integration_map_cf7.select_list(contact_form_7_name)).fill(list_name);

		await this.page.locator(selector.integration_map_cf7.select_list(contact_form_7_name)).press("Enter");

		await this.page.locator(selector.integration_map_cf7.override(contact_form_7_name)).click();

		await this.page.locator(selector.integration_map_cf7.select_name(contact_form_7_name)).selectOption("first_name");

		await this.page.locator(selector.integration_map_cf7.select_email(contact_form_7_name)).selectOption("email");

		await this.page.locator(selector.integration_map_cf7.save_settings).click();

		expect(await this.page.locator(selector.integration_map_cf7.success_toast).innerText()).toEqual("Settings saved successfully");
	}
...
------WebKitFormBoundary66t6AAYgRH37yFnA--`;
	...
	async delete_contact_form_7(contact_form_7_name: string) {
		...
		await this.page.locator(selector.integration_delete_cf7.select_cf7(contact_form_7_name)).click();

		await this.page.locator(selector.integration_affiliate_actions.select_bulk_action).selectOption("Delete");
		await this.page.locator(selector.integration_affiliate_actions.do_action).click();
		await this.page.waitForLoadState("networkidle");
		expect(await this.page.locator(selector.integration_delete_cf7.delete_toast).isVisible()).toBeTruthy();
	}
	...
	async woocom_product_create(product_name: string) {
		const woocom_product_create = await this.request.post(`${data.rest_url}/wc/v3/products`, {
			headers: {
				Authorization: `Basic ${Buffer.from(`${data.woocom_rest_api.consumer_key}:${data.woocom_rest_api.consumer_secret}`).toString("base64")}`,
			},
			data: {
				name: product_name,
				type: "simple",
	...
	async wperp_crm_contact_create(contcat_user_email: string) {
		let page_url: string = selector.integration_wperp.crm_contact_create_page_url;
		let locator: string = selector.integration_wperp.contact_create_nonce_locator;
		let popup_locator: string = selector.integration_wperp.contact_create_popup_nonce_locator;

		const wpsite = new WPSitePage(this.page);
		let header = await wpsite.wordpress_nonce_cookie(page_url, locator, true, popup_locator);
		...
		try {
			expect(wperp_crm_contact_create_response.data.data.email).toEqual(contcat_user_email);
	...
	async wperp_crm_contact_delete(wperp_crm_customer_id: string) {
		let page_url: string = selector.integration_wperp.crm_contact_create_page_url;
		let locator: string = selector.integration_wperp.contact_delete_nonce_locator;
	...
	async create_wp_user(username: string, user_email: string) {
		...
		await this.page.locator(selector.wp_user.email).fill(user_email.toLowerCase());
