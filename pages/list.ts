...
export class ListPage {
	...
	async list_create(list_name: string) {
		const list_create = await this.request.post(`${config.use?.baseURL}/v1/lists`, {
			data: {
				name: list_name,
		...
		try {
			expect(list_create_response.message).toEqual("List created successfully.");
	...
	async lists_of_list(list_id: string) {
		const lists_of_list = await this.request.get(`${config.use?.baseURL}/v1/lists`);
	...
	async list_details(list_id: string) {
		const list_details = await this.request.get(`${config.use?.baseURL}/v1/lists/${list_id}`);
	...
	async list_update(list_id: string, list_name: string) {
		const list_update = await this.request.put(`${config.use?.baseURL}/v1/lists/${list_id}`, {
			data: {
		...
		try {
			expect(list_update_response.message).toEqual("List updated successfully.");
		} catch (err) {
	...
	async list_delete(lists: string[]) {
		const list_delete = await this.request.post(`${config.use?.baseURL}/v1/lists`, {
			data: {
				ids: lists,
				_method: "DELETE",
			},
		...
		try {
			expect(list_delete_response.message).toEqual("Lists deleted successfully");
		} catch (err) {
	...
	async enable_double_opt_in(list_id: string) {
		const enable_double_opt_in = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/settings`, {
			data: {
		...
		const base = new BasePage();
		enable_double_opt_in_response = await base.response_checker(enable_double_opt_in);

		try {
			expect(enable_double_opt_in_response.message).toEqual("List settings updated");
		} catch (err) {
	...
	async tag_create(list_id: string, tag_name: string) {
		const tag_create = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/tags`, { data: { name: tag_name } });
	...
	async tag_update(list_id: string, tag_id: string, tag_update_data: {}) {
		const tag_update = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/tags/${tag_id}`, { data: tag_update_data });
	...
	async tag_assign(list_id: string, tag_id: string, subscriber_id: string) {
		const tag_assign = await this.request.put(`${config.use?.baseURL}/v1/subscribers/${subscriber_id}`, {
			data: {
				tags: {
					[list_id]: [tag_id],
				},
	...
	async tag_delete(list_id: string, tag_id: string) {
		const tag_delete = await this.request.delete(`${config.use?.baseURL}/v1/lists/${list_id}/tags/${tag_id}`);
	...
	async segment_create_with_tag_assign(list_id: string, tag_id: string, segment_name: string) {
		...
		form_data.append("segment[segments][0][selectedOperator]", "contact_is_tagged");
		form_data.append("segment[segments][0][selectedSegment]", "tags");
		form_data.append("segment[segments][0][value]", tag_id);

		const segment_create_with_tag_assign = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/segments`, {
			headers: {
		...
		const base = new BasePage();
		segment_create_with_tag_assign_response = await base.response_checker(segment_create_with_tag_assign);

		try {
			expect(segment_create_with_tag_assign_response.data.segment.segments[0].value).toEqual(tag_id);
	...
	async segment_create_with_email_equal(list_id: string, subscriber_email: string, segment_name: string) {
		let form_data = new URLSearchParams();
		// URLSearchParams() - Used to construct form data for requests that use the "application/x-www-form-urlencoded" as "Content-Type"
		...
		const segment_create_with_email_equal = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/segments`, {
			headers: {
				"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
			},
			data: form_data.toString(),
		...
		const base = new BasePage();
		segment_create_with_email_equal_response = await base.response_checker(segment_create_with_email_equal);

		try {
			expect(segment_create_with_email_equal_response.data.segment.segments[0].value).toEqual(subscriber_email);
	...
	async segment_update(list_id: string, segment_id: string, updated_segment_name: string) {
		const segment_update = await this.request.put(`${config.use?.baseURL}/v1/lists/${list_id}/segments/${segment_id}`, { data: { name: updated_segment_name } });
	...
	async filter_segmented_subscribers(list_id: string, segment_id: string, subscriber_id: string) {
		const filter_segmented_subscribers = await this.request.get(`${config.use?.baseURL}/v1/lists/${list_id}/subscribers?segment=${segment_id}`);

		let filter_segmented_subscribers_response: { data: Array<{ id: string }> };

		const base = new BasePage();
		filter_segmented_subscribers_response = await base.response_checker(filter_segmented_subscribers);

		let counter: number = 0;

		try {
			if (filter_segmented_subscribers_response.data.length > 0) {
				for (let i: number = 0; i < filter_segmented_subscribers_response.data.length; i++) {
					if (filter_segmented_subscribers_response.data[i].id == subscriber_id) {
						counter = 1;
						break;
					}
				}
	...
	async segment_delete(segment_id: string) {
		const segment_delete = await this.request.delete(`${config.use?.baseURL}/v1/segments/${segment_id}`);
	...
	async create_custom_field(
		...
		const create_custom_field = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/fields`, {
		...
		const base = new BasePage();
		create_custom_field_response = await base.response_checker(create_custom_field);

		try {
			expect(create_custom_field_response.data.list_id).toEqual(list_id);
			expect(create_custom_field_response.data.slug).toEqual(custom_field_create_data.slug);
		} catch {
	...
	async update_custom_field(
		...
		const update_custom_field = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/fields/${custom_field_data.slug}`, {
			data: {
		...
		const base = new BasePage();
		update_custom_field_response = await base.response_checker(update_custom_field);

		try {
			expect(update_custom_field_response.data.list_id).toEqual(list_id);
			expect(update_custom_field_response.data.meta.options.pop()).toEqual(custom_field_options_new_value);
		} catch {
	...
	async assign_custom_field(list_id: string, subscriber_id: string, custom_field_slug: string, custom_field_meta_options: Array<string>) {
		const assign_custom_field = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/subscribers/${subscriber_id}/fields/${custom_field_slug}`, {
			data: {
				_method: "PATCH",
		...
		const base = new BasePage();
		assign_custom_field_response = await base.response_checker(assign_custom_field);
		...
		try {
			...
			for (let key in assign_custom_field_response.data.fields) {
				if (assign_custom_field_response.data.fields.hasOwnProperty(key)) {
					expect(assign_custom_field_response.data.fields[key]).toEqual(custom_field_meta_options);
				}
			}
		} catch {
	...
	async unassign_custom_field(list_id: string, subscriber_id: string, custom_field_slug: string) {
		const unassign_custom_field = await this.request.post(`${config.use?.baseURL}/v1/lists/${list_id}/subscribers/${subscriber_id}/fields/${custom_field_slug}`, {
			data: {
				_method: "PATCH",
				value: [],
			},
		...
		const base = new BasePage();
		unassign_custom_field_response = await base.response_checker(unassign_custom_field);
		...
		try {
			...
			for (let key in unassign_custom_field_response.data.fields) {
				if (unassign_custom_field_response.data.fields.hasOwnProperty(key)) {
					expect(unassign_custom_field_response.data.fields[key].length).toEqual(0);
				}
			}
		} catch {
	...
	async delete_custom_field(list_id: string, custom_field_slug: string) {
		const delete_custom_field = await this.request.delete(`${config.use?.baseURL}/v1/lists/${list_id}/fields/${custom_field_slug}`);
		...
		const base = new BasePage();
		delete_custom_field_response = await base.response_checker(delete_custom_field);
