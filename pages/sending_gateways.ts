...
export class GatewayPage {
	...
	async connect_gateway(
		...
		const connect_gateway = await this.request.post(`${config.use?.baseURL}/v1/settings/email-outbound`, {
			data: gateway_data,
		});
		...
		try {
			expect(connect_gateway_response.message).toEqual("Settings saved successfully.");
		} catch (err) {
	...
	async set_default_Form_Reply(
		...
		const set_default_Form_Reply = await this.request.post(`${config.use?.baseURL}/v1/settings/email-default-settings`, {
			data: {
		...
		const base = new BasePage();
		set_default_Form_Reply_response = await base.response_checker(set_default_Form_Reply);

		try {
			expect(set_default_Form_Reply_response.message).toEqual("Settings saved successfully.");
		} catch (err) {
