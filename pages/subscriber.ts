...
export class SubscriberPage {
	...
	async subscriber_create(subscriber_email: string, list_id: string, fire_event: number = 0) {
		const subscriber_create = await this.request.post(`${config.use?.baseURL}/v1/subscribers`, {
			data: {
	...
	async subscriber_update(subscriber_updated_data: {}, subscriber_id: string) {
		const subscriber_update = await this.request.put(`${config.use?.baseURL}/v1/subscribers/${subscriber_id}`, {
	...
	async subscriber_delete(list_id: string, subscriber_id: string) {
		...
		const subscriber_delete = await this.request.delete(`${config.use?.baseURL}/v1/lists/${list_id}/delete-subscribers`, {
			headers: {
		...
		try {
			expect(subscriber_delete_response.message).toEqual("Subscribers delete successfully");
		} catch (err) {
	...
	async subscribers_list(list_id: string, subscriber_email: string) {
		const subscribers_list = await this.request.get(`${config.use?.baseURL}/v1/lists/${list_id}/subscribers`, {});
		...
		try {
			if (subscribers_list_response.data.length > 0) {
				for (let i: number = 0; i < subscribers_list_response.data.length; i++) {
	...
	async subscriber_status(list_id: string, subscriber_email: string, subscriber_id: string) {
		const subscriber_status = await this.request.get(`${config.use?.baseURL}/v1/lists/${list_id}/subscribers?s=${subscriber_email}`);
	...
	async subscribe_double_opt_in_list(list_id: string, subscriber_email: string) {
		const subscribe_double_opt_in_list = await this.request.post(`${config.use?.baseURL}/v1/embed/check-subscribe/${list_id}`, { data: { email: subscriber_email, purpose: "test" } });

		let subscribe_double_opt_in_list_response: { url: string } = { url: "" };
		let verification_url: string = "";

		const base = new BasePage();
		subscribe_double_opt_in_list_response = await base.response_checker(subscribe_double_opt_in_list);
	...
	async verify_subscriber(verification_url: string) {
		const verify_subscriber = await this.request.get(`${config.use?.baseURL}${verification_url}`);
		...
		try {
			expect(verify_subscriber_response.message).toContain("Your subscription has been confirmed. You've been added to our list & will hear from us soon.");
		} catch (err) {
	...
	async check_subscriber_on_list(list_id: string, subscriber_email: string) {
		const check_subscriber_on_list = await this.request.get(`${config.use?.baseURL}/v1/lists/${list_id}/subscribers`, {});
		...
		const base = new BasePage();
		check_subscriber_on_list_response = await base.response_checker(check_subscriber_on_list);

		try {
			if (check_subscriber_on_list_response.data.length > 0) {
				for (let i: number = 0; i < check_subscriber_on_list_response.data.length; i++) {
					if (subscriber_email == check_subscriber_on_list_response.data[i].email) {
	...
	async subscriber_tag_details(list_id: string, subscriber_id: string) {
		const subscriber_details = await this.request.get(`${config.use?.baseURL}/v1/subscribers/${subscriber_id}`);
		...
		const base = new BasePage();
		subscriber_details_response = await base.response_checker(subscriber_details);

		try {
			expect(subscriber_details_response.data.id).toEqual(subscriber_id);
			for (let i: number = 0; i < subscriber_details_response.data.lists.length; i++) {
	...
	async subscriber_custom_field_details(list_id: string, subscriber_id: string) {
		const subscriber_custom_field_details = await this.request.get(`${config.use?.baseURL}/v1/subscribers/${subscriber_id}`);
		...
		const base = new BasePage();
		subscriber_custom_field_details_response = await base.response_checker(subscriber_custom_field_details);

		try {
			expect(subscriber_custom_field_details_response.data.id).toEqual(subscriber_id);
			for (let i: number = 0; i < subscriber_custom_field_details_response.data.lists.length; i++) {
				if (subscriber_custom_field_details_response.data.lists[i].id == list_id) {
					subscriber_custom_field_stats.fields_count = Object.keys(subscriber_custom_field_details_response.data.lists[i].fields).length;

					if (subscriber_custom_field_stats.fields_count > 0) {
						for (let key in subscriber_custom_field_details_response.data.lists[i].fields) {
							if (subscriber_custom_field_details_response.data.lists[i].fields.hasOwnProperty(key)) {
								subscriber_custom_field_stats.options_count = subscriber_custom_field_details_response.data.lists[i].fields[key].length;
							}
						}
					}
					break;
